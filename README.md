...
|---|---|---|
| 4 — Event Prover (K=19) | `event_pk.bin`, `event_vk.bin`, `event_config_params.json` | `cargo run --release --bin bridge-event-prove -- --selftest` (~5 min). **Run this first** — the verifier bails on missing `event_vk.bin`. |
...
```bash
cargo run --release --bin bridge-event-prove -- --selftest
```
